...
impl ApiTestService {
    ...
    pub async fn test_single_config(&self, config_id: i64) -> AppResult<TestResult> {
        ...
        .await
        {
            ...
            Ok(Err(e)) => {
                ...
                self.create_failed_result(config_id, latency_ms, &e)
            }
            Err(_) => {
    ...
    async fn perform_api_test(
        ...
        // 构建 Anthropic Messages API 测试请求
        let test_request_body = json!({
            "model": "claude-haiku-4-5-20251001",  // 使用最快最便宜的模型进行测试
            "max_tokens": 10,  // 最小token数，减少成本
            "messages": [{
                "role": "user",
                "content": "Hi"  // 最简单的测试消息
            }]
        ...
        log::debug!("Testing API endpoint: {}", api_url);
        ...
        if status.is_success() {
            // 200 OK - API 密钥有效且请求成功
            log::debug!("API test successful: {}", status);
            Ok(())
            ...
            Err("API 请求配额已耗尽".to_string())
        } else if status.as_u16() == 400 {
            // 400 Bad Request - 可能是请求格式问题，但连接正常
            let error_text = response.text().await.unwrap_or_default();
            log::debug!("API returned 400: {}", error_text);
            // 400 说明服务器有响应，只是请求格式问题，算作连接成功
            Ok(())
        } else {
            // 其他错误
            let error_text = response.text().await.unwrap_or_default();
    ...
    fn save_test_result(&self, result: &TestResult) -> AppResult<i64> {
        self.db_pool.with_connection(|conn| {
            ...
            conn.execute(
                "INSERT INTO TestResult (config_id, group_id, test_at, status, latency_ms, error_message, is_valid_key)
